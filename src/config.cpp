...
Config readConfigFile(std::string file) {
  ...
  for (const auto &node : *project_dependencies) {
    ...
    if (!dependency)
      log << "project.dependencies.x isn't a table" << Log::ERROR;

    // Read dependency name
    auto dep_name = dependency->at_path("name").value<std::string>();
    if (!dep_name)
      log << "project.dependencies.x.name is not defined or isn't a string"
          << Log::ERROR;
    ...
    if (!dep_version)
      log << "project.dependencies.x.version is not defined or isn't a string"
          << Log::ERROR;
    ...
    if (!dep_src)
      log << "project.dependencies.x.src is not defined or isn't a string"
          << Log::ERROR;
